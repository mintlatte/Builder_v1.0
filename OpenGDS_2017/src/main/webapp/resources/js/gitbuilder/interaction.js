...
gb.interaction.SelectWMS.prototype.handleEvent = function(evt) {
	if (evt.type === "singleclick") {
		...
		var that = this;
		var url = this.layer.getSource().getGetFeatureInfoUrl(evt.coordinate, evt.map.getView().getResolution(),
		...
		var param = {
				...
				"bbox" : evt.coordinate[0] + "," + evt.coordinate[1] + "," + evt.coordinate[0] + "," + evt.coordinate[1],
				"format_options" : "callback:getJson"
		};
		var addr = "http://175.116.181.42:9990/geoserver/admin/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=admin%3Ageo_ngi_33611064_A0010000_POLYGON&bbox=153916.9280805453,71921.34185234662,153916.9280805453,71921.34185234662&outputformat=text%2Fjavascript&format_options=callback:getJson";
		$.ajax({
			url : "http://175.116.181.42:9990/geoserver/ows?" + $.param(param),
			dataType : 'jsonp',
			jsonpCallback : 'getJson',
			beforeSend : function(){
				$("#"+evt.map.getTarget()).css("cursor", "wait");
			},
			complete : function(){
				$("#"+evt.map.getTarget()).css("cursor", "default");
			},
			success : function(data){
				var features = new ol.format.GeoJSON().readFeatures(JSON.stringify(data));
				this.features_ = features;
				var layers = that.map_.getLayers();
				for (var i = 0; i < layers.getLength(); i++) {
					if (layers.item(i).get("id") === "temp_vector" && layers.item(i) instanceof ol.layer.Vector) {
						var source = layers.item(i).getSource();
						for (var j = 0; j < features.length; j++) {
							if (!source.getFeatureById(features[j].getId())) {
								source.addFeature(features[j]);
							}	
						}
						break;
					}

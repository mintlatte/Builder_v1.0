...
import com.git.gdsbuilder.FileRead.dxf.TestJTS;
...
public class QA10FileUploadServiceImpl implements QA10FileUploadService {
	...
	public FileMeta test(FileMeta fileMeta) throws Exception {
		...
		// input DB layer
		// LayerInfo returnInfo =
		// dbmanagerService.insertQA10LayerCollection(newCollection, layerInfo);
		GeoLayerInfo returnInfo1 = dbmanagerService.insertQA10LayerCollection(newCollection, layerInfo);

		// publish Layer
		if (returnInfo1 != null) {
			fileMeta.setUploadFlag(true);
			FileMeta geoserverFileMeta = geoserverService.dbLayerPublishGeoserver(returnInfo1);
